...
public class ServiceObjetMetier {
    ...
    public void rechercherClientParDenomination(String denomination, String ville) throws ServiceException {
        try {
            JsonArray jsonListe = new JsonArray();
            String sqlStatement = "SELECT ClientID, TypeClient, Denomination, Adresse, Ville FROM CLIENT WHERE Denomination LIKE ? and Ville LIKE ?";
            List<Object[]> listeClients = this.dBConnection.launchQuery(sqlStatement, "%"+denomination+"%", "%"+ville+"%");
